...
VarList = {'Delta_dB'       50;         % maximum attenuation in RIR computation
           'SilentFlag'     1};         % set to 1 to disable on-screen messages
...
if ~SilentFlag
    PrintLoopPCw(' [ISM_RIR_bank] Computing room impulse responses. '); 
end
for mm = 1:nMics,
    X_rcv = micpos(mm,:);
...
if ~SilentFlag
    fprintf(' [ISM_RIR_bank] RIR bank parameter ''RIR_cell'' saved in file ''%s''\n', RIRFileName);
end
