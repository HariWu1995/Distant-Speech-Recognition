function [AuData] = ISM_AudioData(RIR_cell, SrcSignal, varargin)
...
VarList = {'AudioFileName'      [];         % file name to save audio data
           'Fs'                 [];         % sampling frequency if data 
                                            % to be saved as .wav file
           'AddNoiseFlag'       0;          % Set to 0 if you don't want 
                                            % additive noise
           'NoiseSNR'           20;         % Desired SNR in the resulting 
                                            % signals (in dB)
           'TrajDir'            'SE';       % direction of the source along 
                                            % the trajectory
           'TruncateMicSig'     1;          % truncate mic signals to same 
                                            % length as input signal
           'SilentFlag'         1};         % set to 1 for silent behaviour
...
if ~SilentFlag
    PrintLoopPCw(' [ISM_AudioData] Computing audio data. '); 
end
for tt = 1:nFrames
   ...
   for mm = 1:nMics,    % Compute the received signal by convolving the source signal with the RIR
      if ~SilentFlag
          PrintLoopPCw((tt-1)*nMics+mm,nFrames*nMics); 
      end
      hh = RIR_cell{mm,tt};
...
if ~isempty(AudioFileName) 
    if strcmpi(AudioFileName(end-3:end),'.mat') 
        ...
        if ~SilentFlag
            fprintf(' [ISM_AudioData] Audio data ''AuData'' saved in file ''%s''\n',AudioFileName); 
        end
    else        %strcmpi(AudioFileName(end-3:end),'.wav')),
        foo = max(max(abs(AuData)));
        AuData = AuData*0.99/foo;       % avoid clipping with wavwrite
        audiowrite(AuData,Fs,AudioFileName);
        if ~SilentFlag
            fprintf(' [ISM_AudioData] Audio data saved in file ''%s''\n',AudioFileName); 
        end
    end
end
